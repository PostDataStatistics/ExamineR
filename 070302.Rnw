...
## Linear regression: confidence interval for the intercep, upper limit
...
Determina el extremo SUPERIOR del intervalo de confianza para el t\\'ermino independiente de la recta de regresi\\'on cuando se considera que la variable $y$ como variable respuesta al nivel de confianza $nc=", nc1,"\\%$. Utiliza 4 cifras significativas en tu respuesta.
$$", dataStringX ,"$$
$$", dataStringY ,"$$
    "
 )
...
\begin{solution}
El extremo SUPERIOR del intervalo de confianza pedido es $\Sexpr{sol}$
