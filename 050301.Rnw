...
if(language=='en'){
  question=paste0("$X$ is a normal ramdom variable (unknown variance).  
                    Given a random sample of length $", n,"$ with sample propotion $\\hat p=",p_hat,"$.   Calculate the {\\bf bottom} $a$  of the confidence interval $(a,b)$ for the {\\bf proportion} $p$, with a confidence level $",100*cl,"$\\%. 
