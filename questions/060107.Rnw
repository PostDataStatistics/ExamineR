...
(alpha = sample(c(0.1, 0.05, 0.01), size = 1))
  ...
  (threshold = qnorm(alpha/2, mean = mu0, sd = s/sqrt(n), lower.tail = FALSE))
...
where $\\mu_0=", mu0,"$. What is the threshold value of the sample mean, among those greater than $\\mu_0$,  that leads to reject the 
...
{\\tt qnorm(",alpha/2,", mean = ", mu0,", sd = ",s,"/sqrt(",n,"), lower.tail = FALSE)}
\\end{equation}
