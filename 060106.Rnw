...
(alpha = sample(c(0.1, 0.05, 0.01), size = 1))
  ...
  (threshold = qnorm(alpha/2, mean = mu0, sd = s/sqrt(n), lower.tail = TRUE))
...
where $\\mu_0=", mu0,"$. What is the threshold value of the sample mean, among those smaller than $\\mu_0$, that leads to reject the 
...
Therefore, we reject the null hypothesis if $\\bar X$ is lower than 
\\begin{equation}\\label{1}
\\mu_0 - z_{", alpha/2, "} \\frac{s}{\\sqrt{n}}
\\end{equation}
That is, the boundary of the rejection region. 
Using the data above, it can be straightly calculated as
\\begin{equation}\\label{2}
{\\tt qnorm(",alpha/2,", mean = ", mu0,", sd = ",s,"/sqrt(",n,"), lower.tail = TRUE)}
\\end{equation}
...
are equivalent."
  ...
  question=paste0("$X$ es una variable aleatoria cualquiera. 
              ...
              $$H_0:\\,\\mu_X  ", symbol, " \\mu_0$$
donde $\\mu_0=", mu0,"$. \\textquestiondown A partir de qu\\'e valor de la media muestral, de entre los que son menores que $\\mu_0$,  
...
Esa diferencia es significativa, en las unidades del problema, cuando es mayor que 
                   \\[ z_{", alpha/2, "} \\frac{s}{\\sqrt{n}}\\]
                   Por tanto, se rechaza $H_0$ cuando $\\bar X$ es menor que
                   \\begin{equation}\\label{1}
                   \\mu_0 - z_{", alpha/2, "} \\frac{s}{\\sqrt{n}}
                   \\end{equation}
                   es decir, es la frontera de la regi\\'on de rechazo, que se puede calcula como 
                   \\begin{equation}\\label{2}
                   {\\tt qnorm(",alpha/2,", mean = ", mu0,", sd = ",s,"/sqrt(",n,"), lower.tail = TRUE)}
                   \\end{equation}
                   $$ = ",signif( sol,digits=signifDig),"$$
