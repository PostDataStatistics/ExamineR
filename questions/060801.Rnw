...
while((abs(sol)<0.00001)|(abs(sol)>999)){
    ...
    TestType=1 # Ha: 1 to test sigma1/sigma2 > 1, 2 to test sigma1/sigma2 < 1, 3 to test sigma1/sigma2 = 1
    
    if(TestType==1){
       symbol="\\leq" 
    (s1=signif(sample(seq(0.3,10,length.out=50),1), digits=3))
    (s2=signif(s1/abs(runif(1, min=.6, max=1.6)),digits=3))
    (Statistic=s1^2/s2^2)
    (pValue=1-pf(Statistic,df1=n1-1, df2=n2-1))
    Stat="\\dfrac{s_1^2}{s_2^2}"
    }
    if(TestType==2){
      symbol="\\geq"
    (s1=signif(sample(seq(0.3,10,length.out=50),1), digits=3))
    ...
    (pValue=if(s1^2/s2^2>1){
                                2*(1-pf(Statistic,df1=n1-1, df2=n2-1))
                                } else 
                                {
                                2*(pf(Statistic,df1=n2-1, df2=n1-1))
                                } )
...
if(language=='en'){
  question=paste0("$X_1$ and $X_2$ are a normal random variables.Consider  two random samples of sizes 
                 ...
                 sample standard deviations  $s_1=", s1, ",\\, s_2=", s2,"$.  
                  ...
                  $$H_0: \\sigma_1^2 ", symbol, " \\sigma_2^2 $$
...
Utiliza 4 cifras significativas en tu respuesta."
  ...
  answer=paste0(" El estad\\'istico es $", Stat, "=
