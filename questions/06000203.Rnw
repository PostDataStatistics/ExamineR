...
if(hypothesis == "nula"){ 
     while(abs(p0 - phat) < .1){phat = p0 + sample(seq(-0.3, .3, by = .05), size = 1)} 
...
if(language=='en'){
  ...
  answer=paste0("To be traslated.")
     ...
     if(language=='es'){
  question=paste0("Se quiere comprobar si la proporci\\'on de individuos 
  ...
  answer=paste0("", sol, ". Se rechaza $H_0$ cuando la proporci\\'on muestral $\\hat p=", phat, "$ es significativamente menor que $p_0$.")  
}
cat(question)
@
