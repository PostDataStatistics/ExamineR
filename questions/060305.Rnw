...
# while((abs(sol)<0.0001)|(abs(sol)>999)){
    ...
    TestType = 2 # 1 to test H1: mu > mu0, 2 to test H1: mu < mu0, 3 to test H1: mu = mu0
    if(TestType==1){
...
(sol = signif(p0-qnorm(alpha, lower.tail = FALSE)*sqrt(phat*qhat/n), digits=signifDig))
...
Se repite un experimento de tipo Bernoull\\'i $n=", n,"$ veces  
 la proporci\\'on muestral de  \\'exitos es $\\hat{p}=\\frac{", nhat, "}{", n, "}$. 
              ...
              $$H_0:\\,p  ", symbol, " p_0$$
donde $p_0=", p0,"$. Determina la menor proporci\\'on muestral, al nivel de significaci\\'on $\\alpha =$ ",alpha, ", que se debe observar para rechazar $H_0$. 
Utiliza 4 cifras significativas en tu respuesta.
"
 )

  answer=paste0("Se rechaza $H_0$ si $\\hat p$ es significativamente menor que $p_0$. 
La teor\\'ia dice que 
...
p_0 - z_{", alpha, "} \\sqrt{\\frac{\\hat p \\hat q}{n}}
\\end{equation}
que es la frontera de la regi\\'on de rechazo y se puede calcular como 
\\begin{equation}\\label{2}
{\\tt qnorm(",alpha,", mean = ", p0,", sd = \\sqrt{\\hat p \\hat q / ",n,"}), lower.tail = TRUE)}
\\end{equation}
$$ = ",signif( sol,digits=signifDig),"$$
Es importante entender la equivalencia entre las expresiones (1) y (2).
")
