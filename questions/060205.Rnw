...
el valor umbral (el mayor valor) de la media muestral a partir del que se rechazar $H_0$ en las unidades del problema viene dada por \\begin{equation}\\label{1}
 \\mu_0 - t_{", alpha, ",", n-1 ,"} \\frac{s}{\\sqrt{n}}
\\end{equation}
