...
## Confidence interval (top) for the mean, normal population, known variance, 
...
if(language=='en'){
  question=paste0("$X$ is a normal ramdom variable with (known) variance $\\sigma = ",s^2,"$.  
                    Given a random sample of length $", n,"$ with sample mean $\\bar{X}=",xbar,"$, calculate the {\\bf top} $b$  of the confidence interval $(a,b)$ for the {\\bf mean} $\\mu_X$, with a confidence level $",100*nc,"$\\%.\\
Use ", signifDig, "  significant digits in your answer.")
  
  answer=paste0("The answer is ", sol)
  
}else if(language=='es'){
    question=paste0("La variable aleatoria $X$ es normal con varianza (conociada) $\\sigma = ",s^2,"$. Se ha obtenido una muestra de tama\\~no $", n,"$, con media muestral $\\bar{X}=",xbar,"$. Calcula el {\\bf extremo superior} $b$ del intervalo de confianza $(a,b)$ para la {\\bf media} $\\mu_X$, con un nivel de confianza del $",100*nc,"$\\%.\\
