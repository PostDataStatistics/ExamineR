...
## Confidence interval (top) for the mean, normal population, unknown variance, sample size>30
...
Use ", signifDig, "  significant digits in your answer.")
  ...
  answer=paste0("The answer is ", sol)
    ...
    question=paste0("La variable aleatoria $X$ es normal (con varianza desconociada). Se ha obtenido una muestra de tama\\~no $", n,"$, con media muestral $\\bar{X}=",xbar,"$ y cuasidesviaci\\'on t\\'ipica $s=", s,"$. Calcula el {\\bf extremo superior} $b$ del intervalo de confianza $(a,b)$ para la {\\bf media} $\\mu_X$, con un nivel de confianza del $",100*nc,"$\\%.\\
