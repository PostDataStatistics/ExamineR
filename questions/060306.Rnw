...
Se repite un experimento de tipo Bernoull\\'i $n=", n,"$ veces  
 la proporci\\'on muestral de  \\'exitos es $\\hat{p}=\\frac{", nhat, "}{", n, "}$. 
              ...
              $$H_0:\\,p  ", symbol, " p_0$$
donde $p_0=", p0,"$. Determina, al nivel de significaci\\'on $\\alpha =$ ",alpha, ", la mayor proporci\\'on muestral (de entre las menores que $p_{0}$) que se debe observar para rechazar $H_0$. 
...
\\begin{equation}
{\\tt qnorm(",alpha/2,", mean = ", p0,", sd = \\sqrt{\\hat p \\hat q / ",n,"}), lower.tail = TRUE)}
\\end{equation}
