...
## Confidence interval (bottom) for the mean,  unknown variance, sample size>100
...
if(language=='en'){
  question=paste0("$X$ is a ramdom variable.  
...
Use ", signifDig, "  significant digits in your answer.")
  ...
  answer=paste0("The answer is ", sol)
    ...
    question=paste0("Dada una variable aleatoria $X$ cualquiera, se ha obtenido una muestra de tama\\~no $", n,"$, con media muestral $\\bar{X}=",xbar,"$ y cuasidesviaci\\'on t\\'ipica $s=", s,"$. Calcula el {\\bf extremo inferior} $a$ del intervalo de confianza $(a,b)$ para la {\\bf media} $\\mu_X$, con un nivel de confianza del $",100*nc,"$\\%.\\
