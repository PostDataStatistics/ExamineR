...
(alpha = sample(c(0.1, 0.05, 0.01), size = 1))
  ...
  (threshold = qnorm(alpha, mean = mu0, sd = s/sqrt(n), lower.tail = FALSE))
...
{\\tt qnorm(",alpha,", mean = ", mu0,", sd = ",s,"/sqrt(",n,"), lower.tail = FALSE)}
\\end{equation}
...
el valor umbral (el menor valor) de la media muestral a partir del que se rechazar $H_0$ en las unidades del problema viene dada por \\begin{equation}\\label{1} 
\\mu_0 + z_{", alpha, "} \\frac{s}{\\sqrt{n}}
\\end{equation}
es decir, es la frontera de la regi\\'on de rechazo, que se calcula 
\\begin{equation}\\label{2}
{\\tt qnorm(",alpha,", mean = ", mu0,", sd = ",s,"/sqrt(",n,"), lower.tail = FALSE)}
\\end{equation}
$$ = ",signif( sol,digits=signifDig),"$$
